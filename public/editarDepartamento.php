...
if(isset($_POST['editar'])){
    ...
    $depEdit=new Departamentos();
    $depEdit->setId($id);
    $depEdit->setNom_dep($nombre);
    $depEdit->update();
...
<h2 class="text-white text-center mt-2">Editar Profesor</h2>
    ...
    ?>
        <form class="w-25 mx-auto" name="nt" action="<?php echo $_SERVER['PHP_SELF']; ?>?id={$dep->id}" method="POST">
            <div class="mt-2 ">
                <input type="text" name="nombre" value="<?php echo $dep->nom_dep ?>" required class="form-control" />
            </div>
            <div class="mt-2">
